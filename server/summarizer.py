...
def suggest_image_kwords(company: str, title: Optional[str] = "", model: Optional[str] = "o3-mini-2025-01-31", user_prompt: Optional[str] = None) -> str:
    ...
    if not user_prompt:
        if model == "o3-mini-2025-01-31":
            user_prompt = f"Provide a keyword that can be used to search for an appropriate cover image for an announcement from company {company} with the title: {title}. The keyword should be relevant to the sector the company operates but be vague enough to yield an image appropriate for an article on a news blog. Suggest words that lead to high quality stock images. ONLY PROVIDE THE KEYWORDS WITH NO OTHER JUSTIFICATION OR EXPLANATION. For example, \"Gold Mining\" or \"Uranium Strip Mining\".\n\nKEYWORD:"
        else:
